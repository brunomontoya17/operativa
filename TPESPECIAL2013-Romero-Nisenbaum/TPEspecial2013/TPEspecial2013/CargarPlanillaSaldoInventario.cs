...
{
    public partial class CargarPlanillaSaldoInventario : Form
    {
        public CargarPlanillaSaldoInventario()
        {
            InitializeComponent();
        }
        ...
        private void btnCargarSaldos_Click(object sender, EventArgs e)
        {
                    ...
                    if (desde.Month == hasta.Month)
                    {
                        if (desde.Day == 01)
                        {
                            if (hasta.Day == 31 || hasta.Day == 30 || hasta.Day == 28 || hasta.Day == 29)
                            {
                                ...
                                && dataGridView1.Columns[2].HeaderText.ToLower() == "saldoinventario"
                                && dataGridView1.Columns[3].HeaderText.ToLower() == "unidad")
                                {
                                    foreach (DataGridViewRow item in dataGridView1.Rows)
                                    {
                                        change = c_productos.ProcesarArchivoSaldos(item.Cells["Codigo"].Value, item.Cells["Descripcion"].Value, item.Cells["SaldoInventario"].Value, item.Cells["Unidad"].Value, desde, hasta);
                                    ...
                                    {
                                        MessageBox.Show("Algunos saldos de los productos no han sido guardados");
                                    }
                                }
                                else
                                {
